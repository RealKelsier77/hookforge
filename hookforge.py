...
# Sidebar for inputs (language selection removed)
with st.sidebar:
...
# Templates (English only, language support removed)
hook_templates = {
...
keywords_by_niche = {
    "Finance": ["trick to millions", "investment hack"], 
    "Fitness": ["workout secret", "diet hack"], 
    "Gaming": ["pro tip", "cheat code"],
    "Tech": ["gadget hack", "app secret"], 
    "Motivation": ["mindset shift", "success secret"], 
    "Self-Improvement": ["habit hack", "productivity tip"],
    "Education": ["study hack", "learning trick"], 
    "Vlogs": ["daily hack", "life moment"], 
    "Reactions": ["viral moment", "shock factor"],
    "True Crime": ["case secret", "mystery hack"], 
    "Cooking": ["recipe hack", "cooking tip"], 
    "Travel": ["destination hack", "travel tip"],
    "Beauty": ["makeup hack", "skincare secret"], 
    "DIY": ["craft hack", "project tip"], 
    "Parenting": ["parenting hack", "kid tip"]
}
...
def generate_hooks(niche, tone, video_type, brief, mode):
    templates = tiktok_hook_templates if mode == "TikTok/Shorts" else hook_templates
    templates = templates.get(tone, hook_templates["Exciting"])
...
if st.button("🚀 Generate Hooks & Ideas", key="generate"):
    ...
    hooks = generate_hooks(niche, tone, video_type, video_brief, st.session_state.mode)
    ideas = generate_video_ideas(niche, tone, video_type, video_brief)
    query_params = {"niche": niche, "tone": tone, "video_type": video_type, "mode": st.session_state.mode, "hooks": "|".join([hook[0] for hook in hooks]), "ideas": "|".join(ideas)}
...
# Footer
st.markdown("---")
st.markdown("🎥 **HookForge** - Built with 💖 by [Kelsier] | © 2025", unsafe_allow_html=True)
